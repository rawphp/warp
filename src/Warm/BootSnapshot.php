...
final class BootSnapshot
{
     ...
     * poisons every later test that fires the same event. Restored per sandbox.
     *
     ...
     * lazily once per process, registering their event listeners on the shared
     * dispatcher at that moment. Restoring the dispatcher to its boot snapshot
     * orphans listeners of models booted mid-run, so the memo is rolled back
     * with it — a model first used earlier re-boots in the next sandbox and
     ...
     * snapshot are pruned at every sandbox creation (see restoreOnto() /
     * pruneBaseInstances()).
     *
    ...
    private function __construct(
        ...
        array $baseInstanceKeys,
    ) {
        $this->consoleBootstrappers = $consoleBootstrappers;
        $this->dispatcherListeners = $dispatcherListeners;
        $this->bootedModels = $bootedModels;
        $this->bootedCallbacks = $bootedCallbacks;
        $this->baseInstanceKeys = $baseInstanceKeys;
    }
