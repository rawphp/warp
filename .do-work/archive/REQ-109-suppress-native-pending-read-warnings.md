...
**Terminal state:**
**Parent:**
**Closure proof:** checkpoint_log:passed commit:077c76d — 377 pest passed, pint clean
...
- [x] A pending batch vanishing between scandir and read produces the injected "skipped vanished" warning and NOTHING on the process's native stderr (assert via subprocess with captured real stderr)
- [x] An unreadable pending dir/batch likewise emits only injected warnings
- [x] Skip/warn/retry semantics from REQ-095 are byte-identical (existing tests unchanged)
...
- src/Timing/TimingStore.php — `@`-suppressed the pending-dir `scandir()` in `pendingFiles()` and the pending-batch `file_get_contents()` in `mergedWithPending()`, so a vanished/unreadable batch or dir never leaks a native E_WARNING to real STDERR (bypassing the REQ-101 injected sink); skip/warn/retry semantics and false-handling unchanged
- tests/Unit/Timing/TimingStoreTest.php — 3 subprocess tests (ini_set display_errors=stderr) reproducing finding 11: batch vanishing mid-read, unreadable batch (chmod 0000), unreadable pending dir
