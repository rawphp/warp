...
**Terminal state:**
**Parent:**
**Closure proof:** checkpoint_log:passed commit:1732765
...
- [x] Running `php bin/warp shard 1/2` with both autoload candidates absent prints a single `[warp]` line to stderr mentioning `composer install` and exits 2 — no PHP fatal, no stack trace
- [x] Normal operation with vendor/ present is byte-identical (existing WarpBinTest passes unchanged)
...
- bin/warp — emits `[warp] no autoloader found - run composer install` to STDERR and exits 2 when neither autoload candidate exists, before referencing any autoloaded class
- tests/Integration/Cli/WarpBinTest.php — new integration test reproducing finding 12 (isolated temp tree, asserts exit 2 + `[warp]`/`composer install` stderr, no Fatal error)
