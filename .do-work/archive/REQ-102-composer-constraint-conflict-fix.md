...
**Terminal state:**
**Parent:**
**Closure proof:** checkpoint_log:passed commit:bbc1e02 — 353 pest passed (real vendor)
...
- composer.json — relaxed `phpunit/php-file-iterator` to `^5.0 || ^6.0` (was `^6.0.1`, unsatisfiable for PHPUnit 11.x); added `composer/semver ^3.4` to require-dev
- composer.lock — regenerated (content-hash sync; no installed-version changes)
- tests/Unit/ComposerConstraintTest.php — added phpunit↔file-iterator co-satisfiability guard + composer/semver require-dev presence guard
