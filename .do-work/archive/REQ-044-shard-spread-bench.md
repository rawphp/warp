...
**Parent:**
**Closure proof:** Merged as `merge(REQ-044): shard-spread bench harness`; harness printed count-based vs Warp LPT spread, timing summary read artifacts, `.warp` was removed, full Pest suite passed, and Pint passed.
...
- [x] `bench/shard-spread.php` accepts `<timings-dir> <shards> [paths...]`, rejects missing/invalid args, and exits 2 when no timings exist.
- [x] The report includes file count, total recorded milliseconds, per-shard count-based and Warp LPT predicted durations, spread, and max/mean ratios.
- [x] `bench/shard-spread.sh` records timings with `WARP_TIMINGS=1 WARP_TIMINGS_DIR=.warp/timings ./vendor/bin/pest --parallel "$SUITE"` before running the report.
- [x] Both bench scripts are executable.
- [x] Running the harness against this repo's unit suite produces a table and `php bin/warp timings --timings-dir=.warp/timings` prints stats without errors.
...
1. **runtime** `bench/shard-spread.sh . 4 tests/Unit`
   ...
   - Actual: PASS; report printed 17 files across 4 shards, count-based max/mean 1.655 vs Warp LPT 1.031. Pest parallel returned 1 after printing 137 passed tests, so the wrapper continued because timing artifacts existed and surfaced the warning.
2. **runtime** `php bin/warp timings --timings-dir=.warp/timings`
   - Expected: prints timing artifact stats without errors.
   - Actual: PASS, 137 tests across 17 files.
...
5. **format** `./vendor/bin/pint --dirty`
   - Actual: PASS.
