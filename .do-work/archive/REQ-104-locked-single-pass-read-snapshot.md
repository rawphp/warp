...
**Terminal state:**
**Parent:**
**Closure proof:** checkpoint_log:passed commit:29cfb5d — 367 pest passed, pint clean
**Criteria approved:** agent-drafted
**Priority:** 2
**Size:** M
**Files:** src/Timing/TimingStore.php, src/Cli/ShardCommand.php, tests/Unit/Timing/TimingStoreTest.php, tests/Unit/Cli/ShardCommandTest.php, tests/Integration/Timing/TimingCaptureTest.php
**Depends on:**
...
- [x] One shard invocation scans pending/ and parses each batch exactly once; storedRoot and fileTotals come from the same snapshot (assertable via injected filesystem counters or a spy on the read path)
- [x] With a writable timings dir, the snapshot read holds merge.lock: a merge running concurrently cannot produce a snapshot that mixes pre-merge pending with post-merge timings.json (test via lock-held assertion or interleaving simulation)
- [x] With a read-only timings dir (chmod 555), `warp shard` still succeeds using the lockless fallback and produces the same plan as the writable case for identical store contents
- [x] The read path creates no files in the timings dir other than the lock attempt (read-only dir remains byte-identical after shard)
- [x] All existing TimingStore and ShardCommand tests green
...
- src/Timing/TimingStore.php — memoized single-pass `snapshot()`/`readSnapshot()` shared by load()/storedRoot()/fileTotals(); holds merge.lock for the read with lockless fallback on a read-only dir (UR-011); removed dead `readMerged()`
- src/Cli/ShardCommand.php — comment clarifying storedRoot()+fileTotals() now share one locked snapshot (kills finding-17 TOCTOU)
- tests/Unit/Timing/TimingStoreTest.php — scan-once (finding 17) + lock-blocks-concurrent-read (finding 2) tests; updated 2 assertions (load now locks on writable dir)
- tests/Unit/Cli/ShardCommandTest.php — read-only-dir lockless-fallback test with byte-identical digest + identical plan
- tests/Integration/Timing/TimingCaptureTest.php — updated one assertion (load creates merge.lock on writable dir)
