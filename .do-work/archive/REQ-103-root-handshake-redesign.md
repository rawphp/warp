...
**Terminal state:**
**Parent:**
**Closure proof:** checkpoint_log:passed commit:e22a870 — 361 pest passed (1046 assertions), pint clean
**Criteria approved:** agent-drafted
**Priority:** 3
**Size:** L
**Files:** src/Support/Paths.php, src/Cli/ShardCommand.php, src/Timing/TimingExtension.php, src/Timing/TimingStore.php, src/Shard/SuiteDiscovery.php, tests/Unit/Cli/ShardCommandTest.php, tests/Unit/Timing/TimingStoreTest.php, tests/Unit/Timing/TimingExtensionTest.php, tests/Integration/Timing/TimingCaptureTest.php, tests/Unit/Cli/MergeCommandTest.php, tests/Unit/Support/PathsTest.php
**Depends on:**
...
- [x] Write side and read side compute the root through one shared Paths helper; grep shows no `dirname(realpath` duplication in TimingExtension or ShardCommand and the mirror comment is gone
- [x] A symlinked phpunit.xml (implicit discovery) records and shards with identical roots — shard proceeds duration-balanced, no mismatch (reproduces finding 4 pre-fix)
- [x] `warp shard 1/2 tests --configuration=config/phpunit.xml` run from the project root with timings recorded under root=<project>/config proceeds duration-balanced; the notice states --configuration is ignored for discovery only (reproduces finding 9 pre-fix)
- [x] Merging pending batches with two different roots keeps the artifact's existing root, folds in only matching-root batches, warn-and-deletes the foreign batch under the merge lock, and never mixes key domains (reproduces finding 3 pre-fix); on load(), a foreign-root batch is skip-and-warned and left on disk
- [x] Root mismatch with zero key overlap warns and degrades to count-balanced (exit 0); root mismatch where stored keys match discovered files exits 2 with an error naming both roots
- [x] All existing shard/timing/merge tests green: `./vendor/bin/pest`
...
- src/Support/Paths.php — added `Paths::configRoot()`, the single shared canonical-root resolver
- src/Cli/ShardCommand.php — root via shared resolver in both discovery and explicit modes; discovery-only `--configuration` notice; middle-path mismatch policy; deleted `suiteRoot()`/`absolutePath()`
- src/Shard/SuiteDiscovery.php — added `rootConfigurationPath()` so ShardCommand feeds the resolver the same config file the extension saw
- src/Timing/TimingExtension.php — `canonicalRoot()` delegates to `Paths::configRoot()`; mirror comment removed
- src/Timing/TimingStore.php — root-scoped merge in `mergedWithPending()`: authoritative root, foreign-batch warn-delete (mergeToDisk under lock) / skip-warn+retain (read-only load)
- tests/Unit/Support/PathsTest.php — new `Paths::configRoot` unit tests
- tests/Unit/Cli/ShardCommandTest.php — finding 4/9/7 reproductions; discovery-only notice assertion
- tests/Unit/Timing/TimingStoreTest.php — finding 3 root-scoped merge/load tests
