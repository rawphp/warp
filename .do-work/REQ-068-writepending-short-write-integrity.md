...
- [ ] `writePending()` treats a short write (bytes written < `strlen()` of the encoded JSON) as a failure: the truncated file is NOT renamed into `pending/`, the `.tmp` is cleaned up, and the same failure signal as a `false` return is raised.
- [ ] A complete write still publishes the batch atomically via the existing temp-file-then-rename path — no change to the success behaviour.
- [ ] The fix does not contradict REQ-048's atomic-write design (confirmed by reading the archived REQ); note the reconciliation in the commit body.
- [ ] A new `TimingStoreTest` case simulates a short write and asserts no truncated batch lands in `pending/`.
