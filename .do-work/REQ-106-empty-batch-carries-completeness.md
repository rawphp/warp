...
- [ ] A flush with tests=[] and non-empty completeFiles writes a pending batch containing the completeness map
- [ ] Merging that batch supersedes (removes) the file's stale timing entries from the artifact
- [ ] A flush with tests=[] AND completeFiles=[] still skips the write (no empty junk batches)
- [ ] Integration: a child PHPUnit run of a fully-skipped class whose file has prior recorded timings ends, after `warp merge`, with the stale entries gone
