...
- [ ] README documents registering `RawPHP\Warp\Timing\TimingExtension` in `phpunit.xml`.
- [ ] README documents `WARP_TIMINGS`, `WARP_TIMINGS_DIR`, `.warp/timings`, full-run timing capture, `warp timings`, and `warp shard <k>/<n>`.
- [ ] README explains stdout/stderr behavior, deterministic shard agreement, no-timings fallback, and exit code 3 for empty shards.
- [ ] README public API table includes `WarpMode::timingsEnabled()`, `TimingExtension`, `TimingStore`, `DurationBalancedSharder`, and `bin/warp`.
- [ ] README benchmark commands include `bench/shard-spread.sh /path/to/your/app 8`.
- [ ] CHANGELOG Unreleased includes the S3 timing capture, sharding CLI, env override, Composer binary, and bench harness.
