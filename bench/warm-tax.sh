...
run() { # run <WARP_MODE value> <file count> -> prints elapsed seconds
  rm -rf "$DIR"
  php "$WARP/bench/gen-trivial-tests.php" "$DIR" "$2" 10 >/dev/null
  local t0 t1
  t0=$(php -r 'echo microtime(true);')
  (cd "$APP" && WARP_MODE="$1" ./vendor/bin/pest "$DIR" >/dev/null)
...
for MODE in 0 1; do
  ...
  php -r "printf(\"WARP_MODE=%s  marginal %.2f ms/test  (200 tests: %.2fs, 800 tests: %.2fs)\n\", '$MODE', (($LARGE - $SMALL) / 600) * 1000, $SMALL, $LARGE);"
done
